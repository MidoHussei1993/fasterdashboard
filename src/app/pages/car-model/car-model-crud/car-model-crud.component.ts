...
@Component({
  selector: 'app-car-model-crud',
  templateUrl: './car-model-crud.component.html',
  styleUrls: ['./car-model-crud.component.scss']
})
export class CarModelCrudComponent implements OnInit {
  mode: FormMode;
  form: FormGroup;
  ...
  ngOnInit(): void {
    if(this.mode == FormMode.Edit || this.mode == FormMode.View){
      this.getCarModelById(this.route.snapshot.params.id)
    }
  }
  ...
  getCarModelById(id:number){
    ...
    this.spinner.show();
    this.carModelService.getByID(id).subscribe(res => {
      this.spinner.hide();
  ...
  create() {
    ...
    this.spinner.show();
    this.carModelService.create(body).subscribe(result => {
      this.form.reset();
  ...
  edit() {
    let body = this.form.value;
    this.spinner.show();
    this.carModelService.update(body).subscribe(result => {
      this.spinner.hide();
