...
@Component({
  selector: 'app-transport-order-crud',
  templateUrl: './transport-order-crud.component.html',
  styleUrls: ['./transport-order-crud.component.scss']
})
export class TransportOrderCrudComponent implements OnInit {
...
TransportOrderDetailsDataReport(){
  ...
  this.spinner.show();
  this.reportService.TransportOrderDetailsDataReport(this.route.snapshot.params.id)
  .subscribe((res:any) => {
    this.spinner.hide();
...
getStatusDropdown():void{
  this.orderStatusService.TransportOrderStatusDDL().subscribe((res) => {
    this.statusList = res
  },err => {
...
edit() {
  let body = this.form.value;
  this.spinner.show();
  this.orderStatusService.UpdateTransportOrderStatus(body).subscribe(result => {
    this.spinner.hide();
